...
export default function CreateCampaign() {
  ...
  useEffect(() => {
    ...
    async function checkAuthAndProfile() {
      ...
      try {
        ...
        if (!data?.isComplete) {
          toast.error(
            "Please complete your profile before creating a campaign",
          );
          navigate("/dashboard/profile", { replace: true });
        }
      } catch {
        // profile error is non-fatal here
      }
    }
  ...
  const handleFileSelect = (e) => {
    ...
    for (const file of selectedFiles) {
      ...
      if (file.size > limit) {
        toast.error(
          `"${file.name}" exceeds the ${isVideo ? "20MB" : "10MB"} limit`,
        );
        return;
      }
    }
  ...
  const submitCampaign = async () => {
    ...
    try {
      ...
      if (campaignId) {
        await api.deleteCampaign(campaignId).catch(() => {});
      }
      const msg =
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-400">
            Fill in the details to start your campaign
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-[#13131A] rounded-xl p-6 sm:p-8 border border-[#1a2b6b]">
            ...
            <p className="text-xs text-gray-400 mb-4">
              Supported: Images, PDFs, Documents (max 10MB each) · Videos (max
              20MB each) · Up to 10 files.
            </p>
            ...
            {/* File List */}
            {files.length > 0 && (
              <div className="mt-6 grid grid-cols-3 sm:grid-cols-2 gap-2 sm:gap-4">
                {files.map((fileObj) => (
                  <div
                    key={fileObj.id}
                  ...
                  >
                    {fileObj.type === "image" && fileObj.previewUrl ? (
                      <div className="relative h-14 sm:h-32">
                        <img
                      ...
                      <div className="h-14 sm:h-32 flex items-center justify-center text-gray-400">
                    ...
                    <button
                      ...
                      className="absolute top-1 right-1 sm:top-2 sm:right-2 p-1 sm:p-1.5 rounded-full bg-red-500/90 hover:bg-red-600 transition-colors opacity-0 group-hover:opacity-100"
                    >
            ...
            {files.length < 3 && (
              <p className="mt-4 text-sm text-yellow-500">
                ⚠️ Please upload at least {3 - files.length} more file
                {3 - files.length !== 1 ? "s" : ""}
              </p>
            )}
          </div>
      ...
      {showTerms ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative w-full max-w-2xl bg-[#010410] rounded-xl border border-gray-800/30 overflow-hidden">
            ...
            >
              <p>
                By creating a campaign on Zyra, you confirm that you understand
                and agree to the following terms and consents.
              </p>
              <p>
                You are responsible for the accuracy of your campaign
                information, the lawful use of funds raised, and compliance with
                all applicable laws and platform rules.
              </p>
              <p>
                You confirm that you have the right to raise funds for the
                stated purpose and that your campaign does not infringe on the
                rights of any person or organization.
              </p>
              <p>
                You consent to Zyra processing your campaign data and displaying
                your campaign publicly, including your campaign name, objective,
                files, and fundraising goal.
              </p>
              <p>
                You acknowledge that transactions may be irreversible and that
                Zyra may take action (including pausing or removing campaigns)
                to protect users and platform integrity.
              </p>
              <p>
                You confirm you have read all the text above. Scroll to the end
