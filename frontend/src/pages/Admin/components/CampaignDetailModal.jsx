...
export default function CampaignDetailModal({ campaignId, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-6">
      ...
      <div className="relative w-full max-w-2xl max-h-[88vh] overflow-y-auto bg-[#010410] rounded-2xl border border-gray-800">
        <button
          onClick={onClose}
          className="absolute top-3 right-3 z-10 p-1.5 rounded-full bg-black/40 text-gray-300 hover:text-white hover:bg-black/60 transition-colors"
        >
          <X size={18} />
        </button>

        {isLoading && <div className="p-16 text-center text-gray-400">Loading…</div>}
        {isError && (
          <div className="p-16 text-center text-gray-400">Failed to load campaign.</div>
        )}
        ...
        {campaign && (
          <>
            {campaign.cover_url ? (
              ...
              <div className="w-full h-48 bg-gradient-to-br from-primary/20 to-secondary/20 flex items-center justify-center text-gray-500">
                No cover photo
              </div>
            )}
